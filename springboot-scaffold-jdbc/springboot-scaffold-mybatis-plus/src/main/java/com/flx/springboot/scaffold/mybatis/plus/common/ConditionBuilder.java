...
public class ConditionBuilder<T> {
    ...
    public static String getTableFiledName(String source) {
        if (ScanFieldAlias.fieldAlias.containsKey(source)) {
            return ScanFieldAlias.fieldAlias.get(source);
        }
    ...
    public ConditionBuilder<T> query(Object model) throws Exception {
        ...
        try {
            ...
            Map<String,Class> filedMap=Arrays.stream(fields).collect(Collectors.toMap(Field::getName, Field::getType));
    ...
    public QueryWrapper<T> build(Boolean flag) throws Exception {
		...
		/*if(conditionIsEmpty()){
			condition.ne("state", State.deleted);
		}*/
